...
int Batle_field[n_fields][field_height][field_width];  //[0][*][*] - основное поле; [1][*][*] - копи€ пол€ дл€ прорисовки пол€ во врем€ установки обьектов
...
int element_number_l = -1; // -1 потому что сразу при входе в функцию делаетс€ ++ и дл€ первого раза значение будет 0
...
char building_type;  // тип здани€. инициализируетс€ при выборе в магазине // передаетс€ в функцию прорисовки/перемещении обьекта
	...
	if (player == 'L')
	{
		if (building_type == '1')
		{
			for (int h = 0; h < Building_height; h++)
			{
				...
				{
					Batle_field[h_cors_f][coordinate_h + h][coordinate_w + w] = coordinate_h + 1;  //дл€ центра +1 потому что берутс€ координаты [0][0]
...
void o_get_shop_actions() // принимает клавиши действий в магазине
{
	...
	do
	{
		...
		{
		case '<': player = 'L'; o_display_marker_in_store(building_type); break;  //дл€ переключени€ игрока в магазине  используй (Shift + <) мен€ет на левого   (фича дл€ тестов)
		case '>': player = 'R'; o_display_marker_in_store(building_type); break;  //дл€ переключени€ игрока в магазине  используй (Shift + >) мен€ет на правого  (фича дл€ тестов)
