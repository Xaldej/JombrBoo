...
char building_type = '1';  // тип здани€. инициализируетс€ при выборе в магазине // передаетс€ в функцию прорисовки/перемещении обьекта
...
void o_get_shop_actions() // принимает клавиши действий в магазине
{
	...
	do
	{
		...
		{
		case '<': player = 'L';  o_display_shop(); break;  //дл€ переключени€ игрока в магазине  используй (Shift + <) мен€ет на левого   (фича дл€ тестов)
		case '>': player = 'R';  o_display_shop(); break;  //дл€ переключени€ игрока в магазине  используй (Shift + >) мен€ет на правого  (фича дл€ тестов)
