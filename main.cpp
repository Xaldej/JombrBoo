...
void o_get_shop_actions() // принимает клавиши действий в магазине
{
	...
	while (true)
	{
		...
		{
		case '<': player = 'L';  o_display_marker_in_store(building_type); break; break;  //дл€ переключени€ игрока в магазине  используй (Shift + <) мен€ет на левого   (фича дл€ тестов)
		case '>': player = 'R';  o_display_marker_in_store(building_type); break; break;  //дл€ переключени€ игрока в магазине  используй (Shift + >) мен€ет на правого  (фича дл€ тестов)
